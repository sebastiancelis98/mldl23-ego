...
torch.cuda.manual_seed(13696641)
...
def main():
    ...
    weight_source_class = torch.ones(num_class).to(device)
    weight_domain_loss = torch.Tensor([1, 1]).to(device)

    if args.weighted_class_loss == 'Y':
        weight_source_class = 1 / torch.Tensor(class_freq).to(device)

    if args.weighted_class_loss_DA == 'Y':
        weight_domain_loss = torch.Tensor([1/num_source_train, 1/num_target_train]).to(device)

    # --- Optimizer ---#
    # define loss function (criterion) and optimizer
    if args.loss_type == 'nll':
        criterion = torch.nn.CrossEntropyLoss(weight=weight_source_class).to(device)
        criterion_domain = torch.nn.CrossEntropyLoss(weight=weight_domain_loss).to(device)
    else:
...
def train(num_class, source_loader, target_loader, model, criterion, criterion_domain, optimizer, epoch, log, log_short, alpha, beta, gamma, mu):
    ...
    for i, ((source_data, source_label), (target_data, target_label)) in data_loader:
        ...
        if args.ens_DA == 'MCD' and args.use_target != 'none':
            _, _, _, _, _, attn_target, out_target, out_target_2, pred_domain_target, feat_target = model(
                source_data, target_data, beta, mu, is_train=True, reverse=True)
    ...
    if args.tensorboard:
        ...
        label_source_1 = 1 * torch.eq(label_source_display,
                                      torch.cuda.LongTensor([1]).repeat(label_source_display.size(0))).long().cuda(non_blocking=True)
        label_source_3 = 2 * torch.eq(label_source_display,
                                      torch.cuda.LongTensor([3]).repeat(label_source_display.size(0))).long().cuda(non_blocking=True)
        label_source_11 = 3 * torch.eq(label_source_display,
                                       torch.cuda.LongTensor([11]).repeat(label_source_display.size(0))).long().cuda(non_blocking=True)

        label_target_1 = 4 * torch.eq(label_target_display,
                                      torch.cuda.LongTensor([1]).repeat(label_target_display.size(0))).long().cuda(non_blocking=True)
        label_target_3 = 5 * torch.eq(label_target_display,
                                      torch.cuda.LongTensor([3]).repeat(label_target_display.size(0))).long().cuda(non_blocking=True)
    ...
    for i, (val_data, val_label) in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            _, _, _, _, _, attn_val, out_val, out_val_2, pred_domain_val, feat_val = model(
                val_data, val_data, [0]*len(args.beta), 0, is_train=False, reverse=False)
