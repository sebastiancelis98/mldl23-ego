...
class TA3N(nn.Module):
    ...
    def domainAlign(self, input_S, input_T, is_train, name_layer, alpha, num_segments, dim):
        ...
        input_source_bn = input_source_bn.view((-1, dim, num_segments) + input_source_bn.size()
                                               [-1:])  # reshape back (e.g. 80 x 512 --> 16 x 1 x 5 x 512)
    ...
    def forward(self, input_source, input_target, beta, mu, is_train, reverse):
        ...
        if self.frame_aggregation == 'avgpool' or self.frame_aggregation == 'rnn':
            ...
            if self.use_bn != 'none':
                feat_fc_video_source_3_1, feat_fc_video_target_3_1 = self.domainAlign(
                    feat_fc_video_source_3_1, feat_fc_video_target_3_1, is_train, 'temconv_1', self.alpha.item(), num_segments, 1)
        ...
        if self.ens_DA == 'MCD':
            ...
            pred_fc_video_target_2 = self.fc_classifier_video_target_2(
                feat_fc_video_target) if self.share_params == 'N' else self.fc_classifier_video_source_2(feat_fc_video_target)
